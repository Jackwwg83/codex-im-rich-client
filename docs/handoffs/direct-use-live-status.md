...
| 2026-05-07 SGT Computer Use provider gap tracking | JAC-274 was created under JAC-235 to track real desktop Computer Use provider evidence and live execution acceptance. Current evidence: `pnpm smoke:computer-use-live` default-skips; the explicit dry-run gate returns `ready_dry_run`; non-dry-run real execution remains intentionally blocked because only `FakeComputerUseProvider` and `UnsupportedComputerUseProvider` are implemented. The generated `TurnStartParams` has no verified dynamic-tool registration field and `ToolsV2` exposes only `web_search` / `view_image`; `docs/phase-6/computer-use-capability-evidence.md` records the follow-up. No real provider will be added until current Codex App Server `item/tool/call` namespace/tool/argument evidence is proven without secrets. |
