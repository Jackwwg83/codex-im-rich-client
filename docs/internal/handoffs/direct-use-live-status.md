...
> usability hardening.
> **Last updated:** 2026-05-09 - Block 4 real Telegram acceptance remains
...
> `pendingApprovals=0`. The IM entry semantics are aligned to Codex App
> project/conversation behavior while preserving App Server native cwd/thread
> implementation: conversation is the first-class App Server thread, project is
> an optional selector, plain text and `/new <task>` can create a Codex default
> conversation via native `thread/start({})`, `/projects` lists configured
> projects plus non-selectable App Server thread-history groups without showing
> local paths, `/use` and `/new 1 <task>` select configured projects by number or
> name without accepting raw paths, and `/threads` / `/switch` use native
> `thread/list` / `thread/resume` when available so IM can take over Codex App
> or CLI-created threads without fabricating reusable project ids. New default
> and native conversation records persist the App Server effective `cwd`
> internally so stored `/switch` fallback can recover them later without
> exposing paths or creating IM-side projects. Local verification for this
> entry-semantics update passed `pnpm lint`, `pnpm typecheck`,
> `pnpm typecheck:tests`, `pnpm test` (163 files, 1530 pass, 1 skip),
> `pnpm protocol:check`, and `git diff --check`. Feishu/Lark
...
> `status=file_sent` with message-id presence only. Inbound DingTalk image
> materialization is also live-accepted: a real DingTalk Desktop image upload
> reached Stream, exposed the live `content.downloadCode` /
...
> thread status updates, plan/diff updates, thread name/goal updates, and
> skills/apps refresh notices. Runtime warning, error, and config-warning
> notices also project into the same redacted status section as message/code
...
> `/cu status` now reports the common Computer Use policy/provider/readiness
> surface without starting a Codex turn or any desktop action: enabled state,
...
> `capabilities.experimentalApi`, and explicit `/cu` turns on Telegram,
...
- **Completed in this effort:**
  ...
  - latest commit - C9 `/start` help states that non-command messages are
    Codex prompts for the current project/conversation and that native file/command/tool
...
- No command may switch project/conversation while an active turn or pending approval
  exists.
...
| 2026-05-07 SGT plan/diff status loop | JAC-266 extends the same common `Codex status` projection to native development lifecycle events: `turn/plan/updated`, `turn/diff/updated`, `thread/name/updated`, `thread/goal/updated`, `thread/goal/cleared`, `skills/changed`, and `app/list/updated`. Summaries report counts/status/title-level information only; raw patch bodies, tokens, JSON payloads, chat IDs, user IDs, and message IDs are not rendered. |
...
| 2026-05-07 SGT Slack T4 slash-command loop | Slack Socket Mode `/codex` slash-command payloads now ack immediately and normalize into existing daemon text: known Codex-native command words become `/status`, `/projects`, `/cwds`, `/threads`, `/use`, `/diagnostics`, `/cu status`, etc.; unknown text remains a normal Codex prompt. Slash-command replies use `chat.postMessage` append semantics because there is no editable originating Slack message. Optional App Home remains deferred. |
...
| Lark `/use codex-im` | Historical evidence: Feishu Web sent `/use codex-im`; SQLite `thread_bindings` gained a Lark row for `codex-im`; bot replied `Using project codex-im`. Current user-facing flow prefers `/projects` then `/use <number>`. | green |
...
3. Codex-native IM command/output surface: green for project/conversation/model,
