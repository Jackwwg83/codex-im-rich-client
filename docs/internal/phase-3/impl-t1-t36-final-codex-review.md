...
Findings:
- [P1] [db-backup.ts](/Users/jackwu/projects/codex-im-rich-client/packages/cli/src/db-backup.ts:101) — SQLite backup is not WAL-safe  
  Explanation: `backupDatabase()` copies only the main DB file, but storage opens SQLite with `journal_mode = WAL` in [database.ts](/Users/jackwu/projects/codex-im-rich-client/packages/storage-sqlite/src/database.ts:55). A live daemon can have committed state in `state.db-wal`, so this backup can silently miss recent callback/session data. Current tests use a dummy file copy path, not an open WAL database.  
...
- [P1] [adapter.ts](/Users/jackwu/projects/codex-im-rich-client/packages/im-telegram/src/adapter.ts:175) — Telegram inbound is not paused before shutdown settlement  
  Explanation: `Daemon.stop()` calls `pauseInbound()` when present before failing pending approvals as `transport_lost` in [daemon.ts](/Users/jackwu/projects/codex-im-rich-client/packages/daemon/src/daemon.ts:373), but `TelegramChannelAdapter` has no pause path. Its emitters still invoke handlers without checking paused/stopped state in [adapter.ts](/Users/jackwu/projects/codex-im-rich-client/packages/im-telegram/src/adapter.ts:323). That leaves a shutdown race where queued Telegram messages/actions can enter routing while broker shutdown settlement is in progress.  
...
- [P2] [install-launchd.mjs](/Users/jackwu/projects/codex-im-rich-client/bin/install-launchd.mjs:76) — Live launchd install can render an unloadable service  
...
- [P2] [callback-tokens.ts](/Users/jackwu/projects/codex-im-rich-client/packages/storage-sqlite/src/callback-tokens.ts:340) — Issued callback tokens are never expired by prune  
...
- [P2] [daemon-status.ts](/Users/jackwu/projects/codex-im-rich-client/packages/cli/src/daemon-status.ts:53) — Status CLI has no producer in daemon code  
