...
Findings:
- [P1] [packages/daemon/src/daemon.ts](<repo>/packages/daemon/src/daemon.ts:953) — Project-level ACL is not enforced
...
- [P1] [packages/daemon/src/daemon.ts](<repo>/packages/daemon/src/daemon.ts:448) — Deny-pattern command approvals bypass SecurityPolicy
...
- [P1] [packages/daemon/src/daemon.ts](<repo>/packages/daemon/src/daemon.ts:657) — Callback target deep-equality is missing
...
- [P1] [packages/daemon/src/daemon.ts](<repo>/packages/daemon/src/daemon.ts:534) — Fire-and-forget action/prune paths lack failure containment
...
- [P2] [packages/daemon/src/daemon.ts](<repo>/packages/daemon/src/daemon.ts:1027) — Stuck-issued sweep can drop the approval flag too early
...
- [P2] [packages/daemon/src/daemon.ts](<repo>/packages/daemon/src/daemon.ts:679) — Callback-level audit requirements are not wired
