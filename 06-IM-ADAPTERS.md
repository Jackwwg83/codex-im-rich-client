...
`dingtalk-stream` stable line. Phase 5 pins `^2.1.5` unless plan review requires
a different version. Older project notes may say `dingtalk-stream-sdk-nodejs`;
that is the upstream SDK repository lineage, not the preferred package name for
new implementation.
